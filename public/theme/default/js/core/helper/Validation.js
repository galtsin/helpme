...
dojo.provide("core.helper.Validation");
require([
    "dojo/dom-form",
    "dojo/dom-attr",
    "dojo/_base/lang",
    "dijit/Tooltip"
], function(domForm, domAttr, lang, Tooltip){
    core.helper.Validation = function(){};
    dojo.declare("core.helper.Validation", null, {
        ...
        checkRequired: function(){
            // Вывести сообщения
            var result = true;
            var formObject = domForm.toObject(this._formNode);
            for(var field in formObject) {
                if(formObject.hasOwnProperty(field)) {
                    if(domAttr.has(this._formNode[field], 'required') && formObject[field].length == 0) {
                        if(!this._responseMessages.hasOwnProperty(field)) {
                            this._responseMessages[field] = [];
                        }
                        this._responseMessages[field].push('Незаполнено обязательное поле');
                        result = false;
                    }
                }
            }
